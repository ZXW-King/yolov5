...
class FocalLoss(nn.Module):
    ...
    def  z__init__(self, loss_fcn, gamma=1.5, alpha=0.25):
...
class LandmarksObj(nn.Module):
    # BCEwithLogitLoss() with reduced missing label effects.
    def __init__(self):
        super(LandmarksObj, self).__init__()
        self.loss_fcn = nn.SmoothL1Loss(reduction='none')

    def forward(self, pred, truel, mask):
        loss = self.loss_fcn(pred*mask, truel*mask)
        loss = torch.mean(loss / (torch.sum(mask) + 1e-6), dim=1)
        loss = torch.sigmoid(loss / (torch.sum(mask) + 1e-6))
        return loss
...
class ComputePlateLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.landmarks_obj = LandmarksObj()
        ...
        if g > 0:
            BCEcls, BCEobj, BCElmobj = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g), FocalLoss(BCElmobj, g)

        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj, self.BCElmobj, self.gr, self.hyp, self.autobalance = BCEcls, BCEobj, BCElmobj, model.gr, h, autobalance
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lbox *= self.hyp['box']
        lobj *= self.hyp['obj']
        lmarkobj *= self.hyp['markobj']
        lcls *= self.hyp['cls']
        lmark *= self.hyp['landmark']

        bs = tobj.shape[0]  # batch size

        loss = lbox + lobj + lmarkobj + lcls + lmark
        return loss * bs, torch.cat((lbox, lobj, lmarkobj, lcls, lmark, loss)).detach()
