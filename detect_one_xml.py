...
def detect_one(model, image_path, device, output_dir, new_root):
    new_folder = ET.Element('image')
    new_folder.attrib['file'] = os.path.split(image_path)[1]
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            for j in range(det.size()[0]):
                ...
                classes=np.argmax(det[j,5:14].cpu().numpy())
                print(classes)
                ...
                new_box.attrib['top'] =str(int((xywh[1]-xywh[3]/2.0) * h))
                new_box.attrib['left'] =str(int((xywh[0]-xywh[2]/2.0) * w))
                new_box.attrib['width'] =str(int(xywh[2] * w))
                new_box.attrib['height'] =str(int(xywh[3] * h))

                new_label = ET.Element('name')
                new_label.text = 'desk_rect'

                part0=ET.Element('part')
                part0.attrib['name']=str(0)
                part0.attrib['x']=str(int(landmarks[0] * w))
                part0.attrib['y']=str(int(landmarks[1] * h))

                part1=ET.Element('part')
                part1.attrib['name']=str(1)
                part1.attrib['x']=str(int(landmarks[2] * w))
                part1.attrib['y']=str(int(landmarks[3] * h))

                part2=ET.Element('part')
                part2.attrib['name']=str(2)
                part2.attrib['x']=str(int(landmarks[4] * w))
                part2.attrib['y']=str(int(landmarks[5] * h))

                part3=ET.Element('part')
                part3.attrib['name']=str(3)
                part3.attrib['x']=str(int(landmarks[6] * w))
                part3.attrib['y']=str(int(landmarks[7] * h))

                new_box.append(new_label)
                new_box.append(part0)
                new_box.append(part1)
                new_box.append(part2)
                new_box.append(part3)
                new_folder.append(new_box)
    try:
...
if __name__ == '__main__':
    ...
    new_root = ET.Element('images')
    new_tree = ET.ElementTree(new_root)
    for image_path in tqdm(img_path_list):
        final_root=detect_one(model, image_path, device, args.output_dir, new_root)
    pretty_xml(final_root, '\t', '\n')
    new_tree.write("a.xml", xml_declaration=True, encoding='utf-8')
