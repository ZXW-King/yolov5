...
from models.yolo_plate import *
from utils.torch_utils import select_device, de_parallel
from utils.general import (check_dataset_prune, check_img_size, check_yaml,
...
def prune(data,
    ...
    if training:  # called by train.py
        ...
        save_dir = save_dir / 'labels' if save_txt else save_dir
        ...
        data = check_dataset_prune(data)  # check
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, stride, opt, pad=pad, rect=pt,
                                       workers=workers, prefix=colorstr(f'{task}: '))[0]
    if val_in_prune:
        results, _, _ = val.val(data,
...
def main():
    ...
    params.pop('half')
    params.pop('dnn')
    results_origin, _, _ = val.val(**params)
