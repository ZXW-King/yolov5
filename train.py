...
def train(hyp, opt, device, tb_writer=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '), dupliNums= opt.duplicate)
    ...
    compute_loss = ComputePlateLoss(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 10) % ('Epoch', 'gpu_mem', 'box', 'obj', 'lmobj', 'cls','landmark', 'total', 'labels', 'img_size'))
        ...
        if rank in [-1, 0]:
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results[:-1]) + lr, tags):
                if tb_writer:
